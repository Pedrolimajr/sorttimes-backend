...
router.get('/:linkId', async (req, res) => {
  try {
    ...
    if (link.tipo === 'votacao' || !link.tipo) {
      ...
      // Para eventos live (Gols/Cartões), busca todos os associados ativos (desbloqueados) cadastrados no clube
      const jogadores = await Jogador.find({ ativo: { $ne: false }, nivel: 'Associado' }).select('nome').lean();
